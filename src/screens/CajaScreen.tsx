...
export function CajaScreen() {
  ...
  const usersQuery = useQuery({ queryKey: ['users'], queryFn: getUsers, enabled: user?.role === 'admin' });
  ...
  const openMutation = useMutation({ mutationFn: () => abrirCaja(user!.id, asignadoId, Number(montoInicial)), onSuccess: refreshCaja });
  ...
  if (!caja) {
    if (user?.role !== 'admin') return <View className="flex-1 justify-center bg-slate-50 p-4"><Card><Text className="mb-2 text-xl font-semibold text-slate-900">No tienes una caja asignada</Text><Text className="text-slate-600">Solicita a un administrador que abra y te asigne una caja.</Text></Card><Button title="Cerrar sesión" onPress={() => void logout()} variant="secondary" /></View>;
    const cashiers = usersQuery.data?.filter((item) => item.role === 'user') ?? [];
    const valid = Number(montoInicial) > 0 && Boolean(asignadoId);
    return <View className="flex-1 justify-center bg-slate-50 p-4"><Card><Text className="mb-2 text-xl font-semibold text-slate-900">Abrir Caja</Text><Text className="mb-4 text-slate-600">Registra el monto inicial y asigna el cajero.</Text><Input label="Monto inicial" value={montoInicial} onChangeText={setMontoInicial} keyboardType="decimal-pad" placeholder="0.00" required /><Text className="mb-2 text-sm text-slate-700">Usuario asignado *</Text><View className="mb-4 gap-2">{cashiers.map((item) => <Button key={item.id} title={item.name} onPress={() => setAsignadoId(item.id)} variant={asignadoId === item.id ? 'primary' : 'secondary'} />)}</View><Button title="Abrir caja" onPress={() => openMutation.mutate()} loading={openMutation.isPending} disabled={!valid} />{openMutation.isError ? <Text className="mt-3 text-red-600">{(openMutation.error as Error).message}</Text> : null}</Card><Button title="Cerrar sesión" onPress={() => void logout()} variant="secondary" /></View>;
  }
